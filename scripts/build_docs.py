...
def scan_docs(docs_dir):
    ...
    for root, dirs, files in os.walk(base):
        ...
        for f in files:
            if f == MANIFEST_NAME or not is_doc(f): continue
...
def main():
    ...
    if not os.path.isdir(docs_dir):
        print(f"[ERRORE] Cartella non trovata: {docs_dir}", file=sys.stderr); return 2
    ...
    for d in manifest['docs']:
        c = (d.get('category') or '—'); cats[c] = cats.get(c, 0) + 1
