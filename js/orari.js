...
(function () {
  ...
  function buildFilters() {
    // squadre
    const teams = Array.from(new Set(state.all.map(x => x.team).filter(Boolean)))
    ...
    if (selTeam) {
      ...
      selTeam.addEventListener('change', () => {
        state.team = selTeam.value; applyFilters(); render();
      });
    }
    ...
    if (selDay) {
      selDay.addEventListener('change', () => {
        state.day = selDay.value; applyFilters(); render();
      });
    }
    ...
    if (q) {
      q.addEventListener('input', () => {
        state.q = q.value; applyFilters(); render();
      });
    }
  }
  ...
  function applyFilters() {
    ...
    // ordina per giorno e poi per orario
    rows.sort((a, b) => {
  ...
  function fmt(hhmm) {
    ...
    return hhmm.replace(':', '.'); // es. 18:30 → 18.30
    }
